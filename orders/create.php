...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !$error) {
    ...
    if (!$error && (!$serviceList || $basePrice <= 0 || $quantity <= 0)) {
        $error = 'Заполните все поля корректно. Количество должно быть больше нуля.';
    }
        ...
        <?php elseif ($service): ?>
            <form method="POST" class="order-form">
                <div class="form-section">
                    ...
                    <div class="form-group">
                        <label for="quantity">Количество (листы / услуги):</label>
                        <input type="number" name="quantity" id="quantity" min="1" step="1" value="1" required>
                        <input type="hidden" name="base_price" value="<?= $service['base_price'] ?>">
                        <p><small>Цена за единицу: <?= formatPrice($service['base_price']) ?>. Итоговая стоимость будет рассчитана как цена × количество.</small></p>
                    </div>
                
                <?php if (!empty($addresses)): ?>
                <div class="form-group">
                    <label for="address_id">Адрес доставки (необязательно):</label>
